...
class Cars{
    ...
    static instance : Cars;

    private constructor(name: string, model: number, inventor:string){
        this.name = name;
        this.model = model;
        this.inventor = inventor;
    }
    static createInstance(): Cars{
        if(!Cars.instance){
...
let sum =(...val:number[])=>{
    return val.reduce(function(a: number,b: number){
        return a+b;
    })
